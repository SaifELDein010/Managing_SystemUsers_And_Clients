...
{
    internal class Adding_New_Client_UI
    {
        static private void ReadClientInfo()
        {
            ...
            Console.Write("\t  account name of client: ");
        ...
        static public void AddingClient()
        {

            Managing_Client_UI.Header("Adding new client");
            ReadClientInfo();
            ...
            while (Client_Is_Exist.IsExist(Managing_Client_UI.client.accountName))
            {
                ...
                Console.Write("\t  Another account name: ");
                Managing_Client_UI.client.accountName = Console.ReadLine();
            ...
            Console.Write("\t Ask client if he/she sure to adding this client account?(Y/N): ");
            if (!Managing_Client_UI.CountiesProcess())
                return;


            if (Adding_New_Client.AddingNewClient(Managing_Client_UI.client) != 911)
            {
                Console.WriteLine("\t There is an error in system");
                return;
            }

            Console.WriteLine("\n\t Client adding successful");
